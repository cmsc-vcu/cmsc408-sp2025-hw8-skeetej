...
overall_total AS (SELECT COUNT(*) AS total FROM wdi_country WHERE `Region` IS NOT NULL)
SELECT 
  `Region`,
  `Income Group`,
  country_count,
  ROUND((country_count / (SELECT total FROM overall_total)) * 100, 2) AS percent_of_total
FROM counts;
""")
...
SELECT 
  `Income Group`,
  count,
  ROUND((count / (SELECT total FROM overall_total)) * 100, 2) AS percent
FROM income_totals
