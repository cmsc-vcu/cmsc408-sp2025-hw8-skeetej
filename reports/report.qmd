...
SELECT 
  ...
  c.`Income Group`,
  c.country_count,
  ROUND((c.country_count / r.region_total) * 100, 1) AS region_percent
FROM counts c
...
SELECT
  `Region`,
  SUM(CASE WHEN `Income Group` = 'High income' THEN country_count ELSE 0 END) AS High_Income,
  ...
  SUM(CASE WHEN `Income Group` = 'Low income' THEN country_count ELSE 0 END) AS Low_Income,
...
region_totals AS (
  SELECT `Region`, SUM(country_count) AS total
  FROM income_counts
  GROUP BY `Region`
)
SELECT
  i.`Income Group`,
  SUM(i.country_count) AS Total_Countries,
  ROUND((SUM(i.country_count) / (SELECT COUNT(*) FROM wdi_country WHERE `Region` IS NOT NULL)) * 100, 1) AS Global_Percent
FROM income_counts i
GROUP BY i.`Income Group`
